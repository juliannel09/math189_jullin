...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X @ W)
	trans = np.multiply(y, np.log(mu)) + np.multiply((1. - y), np.log(1. - mu))
	nll = (-sum(trans) + reg / 2 * np.linalg.norm(W) ** 2).item(0)
...
def get_description(X, y, W):
	...
	pred = predict(X, W)
	m, n = X.shape
	acc_count, prec_count, recall_count = 0, 0, 0
	prec_total, recall_total = 0, 0
	for i in range(m):
		actual, prediction = y.item(i), pred.item(i)
		...
		if prediction == 1:
			...
			if actual == 1:
				prec_count += 1
	accuracy = 1. * acc_count / m
	precision = 1. * prec_count / prec_total
	recall = 1. * recall_count / recall_total
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	# checked solution for a list of lambda values
	reg_list = [0., 0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 30.0]
	reg_list.sort()
	f1_list = []
	# Newton's method or gradient descent
