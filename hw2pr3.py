...
def linreg(X, y, reg=0.0):
	...
	identity[0,0] = 0 # I checked the sol for this because my code was buggy
...
def find_RMSE(W, X, y):
	...
	diff = y-yPred
	m = X.shape[0]
	RMSE = np.sqrt(np.linalg.norm(diff, 2) ** 2/m) 
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort() # Also checked sol here bc was buggy
	W_list = [linreg(X_train, y_train, reg = b) for b in reg_list]
	for count in range(len(reg_list)):
		W_opt = W_list[count]
	...
	"*** YOUR CODE HERE ***"
	opt_lIndex = np.argmin(RMSE_list)
	reg_opt = reg_list[opt_lIndex]
	"*** END YOUR CODE HERE ***"
	return reg_opt
...
def linreg_no_bias(X, y, reg=0.0):
	...
	m = X.shape[0]
	diag = np.eye(m)
	total = X.T @ (np.eye(m) - np.ones(m)/m)
	W_opt = np.linalg.solve(total @ X + reg*np.eye(total.shape[0]), \
		total @ y)
...
if __name__ == '__main__':
	...
	# Fill in code in linreg, findRMSE, and RMSE_vs_lambda for this step
	...
	# Fill in code in norm_vs_lambda for this step
	...
	# Fill in code in linreg_no_bias for this step
	...
	# Fill in code in grad_descent for this step
